...
''' data '''
# you should prepare your own data in ./data/simpsons
# simpsons original size is [120, 160, 3]


def preprocess_fn(img):
    crop_size = 120
    re_size = 64
    img = tf.image.crop_to_bounding_box(img, (120 - crop_size) // 2, (160 - crop_size) // 2, crop_size, crop_size)
    img = tf.to_float(tf.image.resize_images(img, [re_size, re_size], method=tf.image.ResizeMethod.BICUBIC)) / 127.5 - 1
    return img

img_paths = glob.glob('./data/simpsons/*.jpg')
data_pool = utils.DiskImageData(img_paths, batch_size, shape=[120, 160, 3], preprocess_fn=preprocess_fn)
...
with tf.device('/gpu:%d' % gpu_id):
    ...
    # generate
    fake = generator_vanilla(z, reuse=False)

    # dicriminate
    r_logit = discriminator_vanilla(real, reuse=False)
    f_logit = discriminator_vanilla(fake)

    # losses
    d_r_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=r_logit, labels=tf.ones_like(r_logit)))
    d_f_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=f_logit, labels=tf.zeros_like(f_logit)))
    ...
    # sample
    f_sample = generator_vanilla(z, training=False)
...
# summary writer
summary_writer = tf.summary.FileWriter('./summaries/simpsons_vgan', sess.graph)

''' initialization '''
ckpt_dir = './checkpoints/simpsons_dcgan'
...
try:
    ...
    for it in range(sess.run(it_cnt), max_it):
        ...
        if (it + 1) % 100 == 0:
            ...
            utils.imwrite(utils.immerge(f_sample_opt.reshape(re_size, re_size), 10, 10), '%s/Epoch_(%d)_(%dof%d).jpg' % (save_dir, epoch, it_epoch, batch_epoch))
